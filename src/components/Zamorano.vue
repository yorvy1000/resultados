<template>
  ...
  <img src="/images/signos/ZAMORANO.png" alt="Imagen" class="imagen-loteria" />
  <h3>Triple Zamorano</h3></div>
   <div class="loteria-info" v-if="loterias.length > 0">
     <div class="horarios-adicionales">

    <!--   <div v-if="loterias.length > 0">
      <pre>{{ loterias }}</pre> 
    </div>
   ...
   async consultarResultados() {
     ...
     try {
       ...
       const response = await axios.post('https://apiresultados.banklot.com.ve/api/result/getResultTP', {
         ...
         loteria: '9', // Código de la lotería ZAMORANO
         producto: '3' // Código de producto para Triple ZAMORANO
       }, {
         headers: {
